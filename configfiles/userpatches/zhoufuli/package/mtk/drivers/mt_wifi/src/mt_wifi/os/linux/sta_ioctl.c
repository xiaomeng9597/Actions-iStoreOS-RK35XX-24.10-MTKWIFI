					 ...
					 struct iw_freq *freq, char *extra)
{
	...
	if (RTMP_DRIVER_IOCTL_SANITY_CHECK(pAd, NULL) != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "INFO::Network is down!\n");
		return -ENETDOWN;
	}
					 ...
					 __u32 *mode, char *extra)
{
	...
	if (*mode == IW_MODE_ADHOC)
		...
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "===>rt_ioctl_siwmode::SIOCSIWMODE (unknown %d)\n", *mode);
		return -EINVAL;
	}
...
#define PRISM_TYPE 1
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "==>rt_ioctl_giwmode(mode=%d)\n", *mode);
	return 0;
}
					  ...
					  struct iw_point *data, char *extra)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "===>rt_ioctl_giwrange\n");
	...
	struct iw_point *dwrq,	char *extra)
{
	...
	len = dwrq->length;
	os_alloc_mem(NULL, (UCHAR **)&ext, len);
	...
	if (copy_from_user(ext, dwrq->pointer, len)) {
		os_free_mem(ext);
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "andriod_handle_private   copy_from_user\n");
		return -EFAULT;
	}
	...
	if (rtstrcasecmp(ext, "START") == TRUE) {
		...
		/* OK if successful */
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "sSTART Turn on Wi-Fi hardware\n");
		os_free_mem(ext);
		return -1;
	} else if (rtstrcasecmp(ext, "STOP") == TRUE) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "STOP Turn off  Wi-Fi hardware\n");
		os_free_mem(ext);
		return -1;
				   ...
				   struct sockaddr *ap_addr, char *extra)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "IOCTL::SIOCSIWAP "MACSTR"\n",
			 MAC2STR(Bssid));
	return 0;
}
				   ...
				   struct sockaddr *ap_addr, char *extra)
{
	...
	if (RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCGIWAP, 0,
		...
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "IOCTL::SIOCGIWAP(=EMPTY)\n");
		return -ENOTCONN;
	}
					  ...
					  struct iw_point *data, char *extra)
{
	...
	/* allocate memory */
	os_alloc_mem(NULL, (UCHAR **) &(pBssList->pList), sizeof(RT_CMD_STA_IOCTL_BSS) * IW_MAX_AP);

	if (pBssList->pList == NULL) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "Allocate memory fail!!!\n");
		return 0;
	}
	memset(pBssList->pList, 0, sizeof(RT_CMD_STA_IOCTL_BSS) * IW_MAX_AP);
	...
	memset(addr, 0, sizeof(struct sockaddr) * IW_MAX_AP);
					 ...
					 struct iw_point *data, char *extra)
{
	...
	for (i = 0; i < pIoctlScan->BssNr; i++) {
		...
		if (pIoctlScan->pBssTable[i].SupRateLen) {
			...
			if (tmpRate == 0x6c && pIoctlScan->pBssTable[i].has_ht_cap) {
	...
	/*	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO ,"===>rt_ioctl_giwscan. %d(%d) BSS returned, data->length = %d\n",i , pAd->ScanTab.BssNr, data->length); */
...
static int mt_private_meta_set(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrq, char *extra)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "<== %s(), wrq->length = %d\n", __func__, wrq->length);
	GET_PAD_FROM_NET_DEV(pAd, dev);
...
static int mt_private_meta_qurey(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrq, char *extra)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "<== %s(), wrq->length = %d, wrq->pointer:%p\n", __func__, wrq->length, wrq->pointer);
	GET_PAD_FROM_NET_DEV(pAd, dev);
						  ...
						  struct iw_point *wrq, char *extra)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "<== rt_private_get_statistics, wrq->length = %d\n", wrq->length);
	return Status;
}
					 ...
					 char *extra)
{
	...
	default:
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "====> %s - Unknow Command\n", __func__);
		break;
	}
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	UINT64 DefValue = param->value;
	USHORT DefFlag = (param->flags & IW_AUTH_INDEX);
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "[%d] - param->value = %llu!\n", DefFlag, DefValue);
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "param->value = %llu!\n", DefValue);
	return 0;
}
						  ...
						  char *extra)
{
	...
	if (alg == IW_ENCODE_ALG_NONE)
		...
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN, "Warning: Security type is not supported. (alg = %d)\n", alg);
		pIoctlSec->Alg = alg;
		return -EOPNOTSUPP;
	}
					  ...
					  union iwreq_data *wrqu, char *extra)
{
	...
	GET_PAD_FROM_NET_DEV(pAd, dev);
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "===> rt_ioctl_giwencodeext\n");
					  ...
					  char *extra)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "===> rt_ioctl_siwpmksa\n");
					 ...
					 struct iw_point *wrq, char *extra)
{
	...
	RTMP_STA_IoctlHandle(pAd, &wrqin, CMD_RTPRIV_IOCTL_BBP, 0,
						 extra, IW_PRIV_SIZE_MASK, RT_DEV_PRIV_FLAGS_GET(dev));
	wrq->length = wrqin.u.data.length;
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "<==rt_private_ioctl_bbp\n\n");
	return Status;
}
#endif /* DBG */
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	/*	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	if (RTMP_DRIVER_IOCTL_SANITY_CHECK(pAd, NULL) != NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "rt_ioctl_siwrate::Network is down!\n");
		return -ENETDOWN;
	}

	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "rt_ioctl_siwrate::(rate = %d, fixed = %d)\n", rate, fixed);
	/* rate = -1 => auto rate */
...
INT rt28xx_sta_ioctl(void *net_dev_obj, void *rq, INT cmd) /* snowpin for ap/sta */
{
	...
	if (wrqin == NULL) {
		MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_DEBUG, "INFO::wrqin is null!\n");
		return Status;
	}
	...
	case SIOCGIFHWADDR:
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "IOCTL::SIOCGIFHWADDR\n");
