	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	ret = mt_serv_set_freq_offset(serv_test, band_idx);

	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
		("%s: freq offset=%u band_idx:%d\n", __func__,
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	if ((2 + (cr_total_len)) > sizeof(hqa_frame->data)) {
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
		("%s: requested len is longer, make it smaller\n",
		__func__));
		ret = SERV_STATUS_AGENT_INVALID_LEN;
...

{
	...
	CONFIG_SET_PARAM(serv_test, fixed_payload,
			(u_char)fixed_payload, band_idx);

	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
		("%s: fixed_payload =%u\n", __func__, fixed_payload));
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	path_len = chip_cap->mcs_nss.max_path[band_idx][1];
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	sys_ad_move_mem((hqa_frame->data + 2 + sizeof(tx_ant)),
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	u_char *data = hqa_frame->data;
	u_int32 mode = 0, icap_len, resp_len = 2, bypass = 0;
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE, ("%s: mode[%d]\n", __func__, mode));

	/* Set parameters */
	test_config = &serv_test->test_config[serv_test->ctrl_band_idx];

	/* Prevent checking band uninitiilized
		when ctrl_band_idx is changed to band1 in the single card */
	if (test_config->op_mode < fTEST_IDLE || test_config->op_mode > fTEST_IN_ICAPOVERLAP)
		test_config = &serv_test->test_config[0];

	if (test_config) {
		if (mode == fTEST_OPER_NORMAL_MODE) {
			/*  ignore config normal mode when go into testmode first
				avoid bssinfo is released by fw */
			if (test_config->op_mode == fTEST_IDLE)
				bypass = 1;
	...
	if (bypass)
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("\033[1;33m%s: Mode=%d, bypass=%d, band_idx=%d !!!\033[0m\n", __func__,
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR, ("%s:0x%08x\n", __func__, sub_chip_id));

	/* Update hqa_frame with response: status (2 bytes) */
	sys_ad_move_mem(hqa_frame->data + 2, &sub_chip_id, sizeof(sub_chip_id));
	update_hqa_frame(hqa_frame, 2 + sizeof(sub_chip_id), ret);
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	if ((log_type < TEST_LOG_RXV) || (log_type > TEST_LOG_TXSSHOW)) {
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("%s: log_type: %d error!!\n", __func__, log_type));
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	u_char src_addr[SERV_MAC_ADDR_LEN] = {0, 0, 0, 0, 0, 0};
	...
	case 1:
		...
		/* source address lsb */
		icap_info.u4FixRxGainSel = (u_int32)src_addr[0];

		/* source address msb */
		icap_info.u4WifiPath = src_addr[4];
		...
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_TRACE,
		("%s: u4FixRxGainSel = 0x%08x, u4WifiPath = 0x%08x\n"
		, __func__, icap_info.u4FixRxGainSel, icap_info.u4WifiPath));
...
#define SEG_STA_CNT		16
	...
	sys_ad_set_mem(ru_allocation, sizeof(struct test_ru_allocatoin), 0xff);
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	switch (action) {
	case HQA_TXBF_INIT:
		tmplen = snprintf(cmd, HQA_BF_STR_SIZE, "%d", (u_char)arg[0]);
		if (snprintf_error(HQA_BF_STR_SIZE, tmplen)) {
			SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
				("%s: snprintf error!!\n", __func__));
			goto HQA_IBF_CMD_FAIL;
		}
	...
	case HQA_CHANNEL:
		if (arg[1] == 1) {
			tmplen = snprintf(cmd, HQA_BF_STR_SIZE, "%d:1",
								(u_char)arg[0]);
			if (snprintf_error(HQA_BF_STR_SIZE, tmplen)) {
				SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
					("%s: snprintf error!!\n", __func__));
			...
			tmplen = snprintf(cmd, HQA_BF_STR_SIZE, "%d",
								(u_char)arg[0]);
			if (snprintf_error(HQA_BF_STR_SIZE, tmplen)) {
				SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
					("%s: snprintf error!!\n", __func__));
	...
	case HQA_IBF_PHASE_COMP:
		...
		tmplen = snprintf(cmd, HQA_BF_STR_SIZE,
					"%02x:%02x:%02x:%02x:%02x",
	...
	case HQA_IBF_PROF_UPDATE:
		...
		tmplen = snprintf(cmd, HQA_BF_STR_SIZE, "01:00:01:00:01");
		if (snprintf_error(HQA_BF_STR_SIZE, tmplen)) {
			SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
				("%s: snprintf error!!\n", __func__));
			goto HQA_IBF_CMD_FAIL;
	...
	struct service_test *serv_test, struct hqa_frame *hqa_frame)
{
	...
	tmplen = snprintf(cmd, HQA_BF_STR_SIZE,
					...
					h11, angle_h11, h21, angle_h21, h31,
					angle_h31, h41, angle_h41);
	if (snprintf_error(HQA_BF_STR_SIZE, tmplen)) {
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_ERROR,
			("%s: snprintf error!!\n", __func__));
		goto HQA_PROFILE_UPDATE_FAIL;
	...
	struct service_test *serv_test, u_char *arg)
{
	...
	if ((fg_dbdc) && (cap->is_dual_phy == FALSE)) {
		if (ctrl_band_idx == 0) {
			/* set ATETXANT=3 2T */
	...
	s_int8 *hqa_frame_string, struct hqa_frame *hqa_frame)
{
	...
	u_int32 tmp_value_32 = 0;
	u_int16 tmp_value_16 = 0;
	u_int8  tmp_value_8 = 0;
	u_char *data = NULL;
	u_int16 ret = 0;
	u_int8 parasize = 0;
...
s_int32 mt_agent_show_TxDone(struct service_test *serv_test)
{
	...
	SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF, ("%s(): tx done count of band[%d]: %u\n",
				__func__, TEST_DBDC_BAND0, tx_cnt0));
	...
	if (IS_TEST_DBDC(serv_test->test_winfo)) {
		...
		SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF, ("%s(): tx done count of band[%d]: %u\n",
					__func__, TEST_DBDC_BAND1, tx_cnt1));
	}
	return ret;
}
...
s_int32 mt_agent_show_ru_info(struct service_test *serv_test)
{
	...
	for (sta_idx = 0 ; sta_idx < MAX_MULTI_TX_STA ; sta_idx++) {
		if (ru_sta[sta_idx].valid) {
			SERV_LOG(SERV_DBG_CAT_TEST, SERV_DBG_LVL_OFF,
				("(%s) RU index[%u] in Segment[%u]: Allocation[%u]"
				"aid[%u](0: N/A) TX mcs[%u],nss[%u], ldpc[%u], "
				"start spetial stream:%u, mpdu length:%u, alpha:%d\n",
				__func__, (ru_sta[sta_idx].ru_index >> 1),
				(ru_sta[sta_idx].ru_index & 0x1), ru_sta[sta_idx].allocation,
...
s_int32 mt_agent_set_ru_cli(struct service_test *serv_test, u_char *arg)
{
	...
	ru_allocation = CONFIG_GET_PADDR(serv_test, ru_alloc, band_idx);
	argLen = strlen(arg);
	ret = sys_ad_alloc_mem((pu_char *)&pBuff, argLen + 1);
	...
	if (argLen > 0) {
		...
		if (strlen(value) == 3) {
			...
			sumOfSegs = seg_sta_cnt[0] + seg_sta_cnt[1];
			pRUString = NULL;
		} else{
			strncpy(arg, pBuff, argLen + 1);
			pRUString = arg;
		}
		for (i = 0, value = agent_trtok(pRUString, ":");
			value;
			...
			if (strlen(value) > 0 && input_cnt == 9) {
				...
				ru_info[i].ru_index = (ru_info[i].ru_index << 1) | (i < seg_sta_cnt[0] ? 0 : 1);

				if (ru_info[i].mpdu_length <= 24)
					ru_info[i].mpdu_length =
